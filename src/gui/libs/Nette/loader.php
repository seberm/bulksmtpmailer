...
getComponents($deep=FALSE,$filterType=NULL);}}namespace Nette\Application{use
Nette;interface
...
isControlInvalid();}}namespace Nette{use
Nette;interface
...
send(Nette\Web\IHttpRequest$httpRequest,Nette\Web\IHttpResponse$httpResponse);}interface
IRouter{const
ONE_WAY=1;const
SECURED=2;function
match(Nette\Web\IHttpRequest$httpRequest);function
constructUrl(PresenterRequest$appRequest,Nette\Web\Uri$refUri);}}namespace Nette{use
Nette;interface
IFreezable{function
freeze();function
isFrozen();}}namespace Nette\Caching{use
Nette;interface
...
normalizeRow($row,$statement);}}namespace Nette\Forms{use
Nette;interface
...
render(Form$form);}}namespace Nette\Web{use
Nette;interface
IHttpRequest{const
...
getAuthorizationHandler();}}namespace Nette{use
Nette;interface
IContext{function
...
__unset($name){throw
new\MemberAccessException("Cannot unset the property {$this->reflection->name}::\$$name.");}}}namespace Nette\Loaders{use
Nette;final
class
...
DebugHelpers{static
function
renderBlueScreen(\Exception$exception){if(class_exists('Nette\Environment',FALSE)){$application=Environment::getContext()->hasService('Nette\\Application\\Application',TRUE)?Environment::getContext()->getService('Nette\\Application\\Application'):NULL;}static$errorTypes=array(E_ERROR=>'Fatal Error',E_USER_ERROR=>'User Error',E_RECOVERABLE_ERROR=>'Recoverable Error',E_CORE_ERROR=>'Core Error',E_COMPILE_ERROR=>'Compile Error',E_PARSE=>'Parse Error',E_WARNING=>'Warning',E_CORE_WARNING=>'Core Warning',E_COMPILE_WARNING=>'Compile Warning',E_USER_WARNING=>'User Warning',E_NOTICE=>'Notice',E_USER_NOTICE=>'User Notice',E_STRICT=>'Strict',E_DEPRECATED=>'Deprecated',E_USER_DEPRECATED=>'User Deprecated');$title=($exception
instanceof\FatalErrorException&&isset($errorTypes[$exception->getSeverity()]))?$errorTypes[$exception->getSeverity()]:get_class($exception);$expandPath=NETTE_DIR.DIRECTORY_SEPARATOR;$counter=0;if(headers_sent()){echo'</pre></xmp></table>';}?><!DOCTYPE html>
<html>
<head>
...
DebugHelpers::highlightFile($ex->getFile(),$ex->getLine())?></pre>
			...
			<?php if(isset($stack[0]['class'])&&$stack[0]['class']==='Nette\Debug'&&($stack[0]['function']==='_shutdownHandler'||$stack[0]['function']==='_errorHandler'))unset($stack[0])?>
			<?php if($stack):?>
			<div class="panel">
...
htmlSpecialChars(PHP_VERSION." |\n".(isset($_SERVER['SERVER_SOFTWARE'])?$_SERVER['SERVER_SOFTWARE']." |\n":'').(class_exists('Nette\Framework')?'Nette Framework '.Framework::VERSION.' ('.substr(Framework::REVISION,8).')':''))?>">&nbsp;<span>Nette Framework</span></li>
			<?php foreach($panels
...
strtr(Debug::$editor,array('%file'=>rawurlencode($file),'%line'=>$line));}static
function
...
function
clickableDump($dump){return'<pre class="nette-dump">'.preg_replace_callback('#^( *)((?>[^(]{1,200}))\((\d+)\) <code>#m',function($m){return"$m[1]<a href='#' rel='next'>$m[2]($m[3]) ".(trim($m[1])||$m[3]<7?'<abbr>&#x25bc;</abbr> </a><code>':'<abbr>&#x25ba;</abbr> </a><code class="nette-collapsed">');},self::htmlDump($dump)).'</pre>';}}final
class
SafeStream{const
...
new\UnexpectedValueException("Method {$this->reflection->name}::$method() did not return or create the desired component.");}return$component;}}final
function
...
_isCloning(){return$this->cloning;}}}namespace Nette\Forms{use
Nette;class
FormContainer
extends
...
Nette\ArrayHash;$cursor=$values;$iterator=$this->getComponents(TRUE);foreach($iterator
as$name=>$control){$sub=$iterator->getSubIterator();if(!isset($sub->cursor)){$sub->cursor=$cursor;}if($control
instanceof
...
instanceof
FormContainer){$cursor=$sub->cursor->$name=new
Nette\ArrayHash;}}return$values;}function
...
PROTECTION='Nette\Forms\HiddenField::validateEqual';const
...
addProtection($message=NULL,$timeout=NULL){$session=$this->getSession()->getNamespace('Nette.Forms.Form/CSRF');$key="key$timeout";if(isset($session->$key)){$token=$session->$key;}else{$session->$key=$token=Nette\String::random();}$session->setExpiration($timeout,$key);$this[self::PROTECTOR_ID]=new
...
function
receiveHttpData(){$httpRequest=$this->getHttpRequest();if(strcasecmp($this->getMethod(),$httpRequest->getMethod())){return;}if($httpRequest->isMethod('post')){$data=Nette\ArrayTools::mergeTree($httpRequest->getPost(),$httpRequest->getFiles());}else{$data=$httpRequest->getQuery();}if($tracker=$this->getComponent(self::TRACKER_ID,FALSE)){if(!isset($data[self::TRACKER_ID])||$data[self::TRACKER_ID]!==$tracker->getValue()){return;}}return$data;}function
...
DefaultFormRenderer;}return$this->renderer;}function
render(){$args=func_get_args();array_unshift($args,$this);echo
call_user_func_array(array($this->getRenderer(),'render'),$args);}function
__toString(){try{return$this->getRenderer()->render($this);}catch(\Exception$e){if(func_get_args()&&func_get_arg(0)){throw$e;}else{Nette\Debug::toStringException($e);}}}protected
function
...
run(){$httpRequest=$this->getHttpRequest();$httpResponse=$this->getHttpResponse();if($this->allowedMethods){$method=$httpRequest->getMethod();if(!in_array($method,$this->allowedMethods,TRUE)){$httpResponse->setCode(Nette\Web\IHttpResponse::S501_NOT_IMPLEMENTED);$httpResponse->setHeader('Allow',implode(',',$this->allowedMethods));echo'<h1>Method '.htmlSpecialChars($method).' is not implemented</h1>';return;}}$request=NULL;$repeatedError=FALSE;do{try{if(count($this->requests)>self::$maxLoop){throw
new
ApplicationException('Too many loops detected in application life cycle.');}if(!$request){$this->onStartup($this);$session=$this->getSession();if(!$session->isStarted()&&$session->exists()){$session->start();}$router=$this->getRouter();Nette\Debug::addPanel(new
...
setContext(Nette\IContext$context){$this->context=$context;return$this;}final
function
...
storeRequest($expiration='+ 10 minutes'){$session=$this->getSession('Nette.Application/requests');do{$key=Nette\String::random(5);}while(isset($session[$key]));$session[$key]=end($this->requests);$session->setExpiration($expiration,$key);return$key;}function
restoreRequest($key){$session=$this->getSession('Nette.Application/requests');if(isset($session[$key])){$request=clone$session[$key];unset($session[$key]);$request->setFlag(PresenterRequest::RESTORED,TRUE);$this->presenter->sendResponse(new
...
getSnippetId($name=NULL){return'snippet-'.$this->getUniqueId().'-'.$name;}}}namespace Nette{class
DebugPanel
extends
Object
implements
...
ob_get_clean();}}}namespace Nette\Application{use
Nette;class
RoutingDebugger
extends
Nette\DebugPanel{private$router;private$httpRequest;private$routers=array();private$request;function
__construct(IRouter$router,Nette\Web\IHttpRequest$httpRequest){$this->router=$router;$this->httpRequest=$httpRequest;parent::__construct('RoutingDebugger',array($this,'renderTab'),array($this,'renderPanel'));}function
...
/><?php if(empty($this->request)):?>no route<?php else:echo$this->request->getPresenterName().':'.(isset($this->request->params[Presenter::ACTION_KEY])?$this->request->params[Presenter::ACTION_KEY]:Presenter::$defaultAction).(isset($this->request->params[Presenter::SIGNAL_KEY])?" {$this->request->params[Presenter::SIGNAL_KEY]}!":'');endif?>
<?php }function
renderPanel(){?>
...
<?php if(empty($this->request)):?>
	no route
<?php else:?>
	<?php echo$this->request->getPresenterName().':'.(isset($this->request->params[Presenter::ACTION_KEY])?$this->request->params[Presenter::ACTION_KEY]:Presenter::$defaultAction).(isset($this->request->params[Presenter::SIGNAL_KEY])?" {$this->request->params[Presenter::SIGNAL_KEY]}!":'')?>
<?php endif?>
</h1>
...
Nette\Debug::dump($value,TRUE);endif?></td>
		</tr>
		<?php endforeach?>
		</tbody>
		</table>
...
__toString(){try{return$this->component->link($this->destination,$this->params);}catch(\Exception$e){Nette\Debug::toStringException($e);}}}use
...
SIGNAL_KEY='do',ACTION_KEY='action',FLASH_KEY='_fid';public
...
instanceof
RenderResponse&&$this->isControlInvalid()){$this->response->send($this->getHttpRequest(),$this->getHttpResponse());$this->sendPayload();}elseif(!$this->response&&$hasPayload){$this->sendPayload();}}catch(AbortException$e){}if($this->hasFlashSession()){$this->getFlashSession()->setExpiration($this->response
instanceof
...
changeAction($action){if(Nette\String::match($action,"#^[a-zA-Z0-9][a-zA-Z0-9_\x7f-\xff]*$#")){$this->action=$action;$this->view=$action;}else{throw
new
...
new\FileNotFoundException("Layout not found. Missing template '$file'.");}}$this->sendResponse(new
...
redirectUri($uri,$code=NULL){if($this->isAjax()){$this->payload->redirect=(string)$uri;$this->sendPayload();}elseif(!$code){$code=$this->getHttpRequest()->isMethod('post')?Nette\Web\IHttpResponse::S303_POST_GET:Nette\Web\IHttpResponse::S302_FOUND;}$this->sendResponse(new
...
PresenterRequest($presenter,PresenterRequest::FORWARD,$args,array(),array());$this->lastCreatedRequestFlag=array('current'=>$current);if($mode==='forward')return;$uri=$router->constructUrl($this->lastCreatedRequest,$refUri);if($uri===NULL){unset($args[self::ACTION_KEY]);$params=urldecode(http_build_query($args,NULL,', '));throw
new
...
getFlashSession(){if(empty($this->params[self::FLASH_KEY])){$this->params[self::FLASH_KEY]=Nette\String::random(4);}return$this->getSession('Nette.Application.Flash/'.$this->params[self::FLASH_KEY]);}function
setContext(Nette\IContext$context){$this->context=$context;return$this;}final
function
...
getConstructor(){return($ref=parent::getConstructor())?MethodReflection::from($this->getName(),$ref->getName()):NULL;}function
...
__construct($baseDir,Nette\IContext$context){$this->baseDir=$baseDir;$this->context=$context;}function
...
getPresenterClass(&$name){if(isset($this->cache[$name])){list($class,$name)=$this->cache[$name];return$class;}if(!is_string($name)||!Nette\String::match($name,"#^[a-zA-Z\x7f-\xff][a-zA-Z0-9\x7f-\xff:]*$#")){throw
new
...
send(Nette\Web\IHttpRequest$httpRequest,Nette\Web\IHttpResponse$httpResponse){}}class
JsonResponse
extends
Nette\Object
implements
...
new\InvalidArgumentException("Payload must be array or object class, ".gettype($payload)." given.");}$this->payload=$payload;$this->contentType=$contentType?$contentType:'application/json';}final
function
getPayload(){return$this->payload;}final
function
getContentType(){return$this->contentType;}function
send(Nette\Web\IHttpRequest$httpRequest,Nette\Web\IHttpResponse$httpResponse){$httpResponse->setContentType($this->contentType);$httpResponse->setExpiration(FALSE);echo
Nette\Json::encode($this->payload);}}class
RedirectingResponse
extends
Nette\Object
implements
...
send(Nette\Web\IHttpRequest$httpRequest,Nette\Web\IHttpResponse$httpResponse){$httpResponse->redirect($this->uri,$this->code);}}class
RenderResponse
extends
Nette\Object
implements
...
send(Nette\Web\IHttpRequest$httpRequest,Nette\Web\IHttpResponse$httpResponse){if($this->source
instanceof
...
constructUrl(PresenterRequest$appRequest,Nette\Web\Uri$refUri){return
NULL;}function
...
new\OutOfRangeException("Offset invalid or out of range");}array_splice($this->list,(int)$index,1);}}}namespace Nette\Application{use
Nette;class
MultiRouter
extends
Nette\ArrayList
...
NULL;}function
constructUrl(PresenterRequest$appRequest,Nette\Web\Uri$refUri){if($this->cachedRoutes===NULL){$routes=array();$routes['*']=array();foreach($this
...
new\InvalidArgumentException("Second argument must be array or string in format Presenter:action, '$metadata' given.");}$metadata=array(self::PRESENTER_KEY=>substr($metadata,0,$a),'action'=>$a===strlen($metadata)-1?'default':substr($metadata,$a+1));}$this->flags=$flags|self::$defaultFlags;$this->setMask($mask,$metadata);}function
...
constructUrl(PresenterRequest$appRequest,Nette\Web\Uri$refUri){if($this->flags&self::ONE_WAY){return
...
match(Nette\Web\IHttpRequest$httpRequest){if($httpRequest->getUri()->getPathInfo()!==''){return
...
new
PresenterRequest($presenter,$httpRequest->getMethod(),$params,$httpRequest->getPost(),$httpRequest->getFiles(),array(PresenterRequest::SECURED=>$httpRequest->isSecured()));}function
constructUrl(PresenterRequest$appRequest,Nette\Web\Uri$refUri){$params=$appRequest->getParams();$presenter=$appRequest->getPresenterName();if(strncasecmp($presenter,$this->module,strlen($this->module))===0){$params[self::PRESENTER_KEY]=substr($presenter,strlen($this->module));}else{return
...
getDefaults(){return$this->defaults;}}}namespace Nette\Caching{use
Nette;class
Cache
extends
Nette\Object
...
new\InvalidStateException("Cannot create journal file $this->file.");}}else{$writen=fwrite($init,pack('N2',self::FILE_MAGIC,$this->lastNode));fclose($init);if($writen!==self::INT32_SIZE*2){throw
new\InvalidStateException("Cannot write journal header.");}}}$this->handle=fopen($this->file,'r+b');if(!$this->handle){throw
new\InvalidStateException("Cannot open journal file '$this->file'.");}if(!flock($this->handle,LOCK_SH)){throw
new\InvalidStateException('Cannot acquire shared lock on journal.');}$header=stream_get_contents($this->handle,2*self::INT32_SIZE,0);flock($this->handle,LOCK_UN);list(,$fileMagic,$this->lastNode)=unpack('N2',$header);if($fileMagic!==self::FILE_MAGIC){fclose($this->handle);$this->handle=false;throw
new\InvalidStateException("Malformed journal file '$this->file'.");}}function
...
new\InvalidStateException("Cannot load node number $nodeId.");break;}$nodeInfo=$node[self::INFO];unset($node[self::INFO]);foreach($node
...
new\InvalidStateException('Cannot load node number '.($nodeId).'.');++$i;continue;}do{$link=$data[$i];if(!isset($node[$link])){if(self::$debug)throw
...
new\InvalidStateException('Bad index.');unset($toDelete[$searchKey]);continue;}foreach($toDelete
...
new\InvalidStateException("Cannot load node number $id.");break;}$this->arrayAppendKeys($data,$childNode[self::INDEX_DATA]);}return$data;}private
function
...
new\InvalidStateException("Cannot load node number $nodeId.");break;}foreach($links
...
new\InvalidStateException("Node $id has malformed header.");$this->deleteNode($id);}return
...
new\InvalidStateException("Cannot deserialize node number $id.");return
...
new\InvalidStateException('Saving node is bigger than maximum node size.');}else{$this->bisectNode($id,$node);return
...
new\InvalidStateException("Cannot write node number $id to journal.");}}private
function
...
new\InvalidStateException("Cannot load node number $parent.");}else{$parentNode[$halfKey]=$firstId;ksort($parentNode);$this->saveNode($parent,$parentNode);}}}private
function
...
new\InvalidStateException('Cannot truncate journal file.');}}else{fseek($this->handle,self::HEADER_SIZE+self::NODE_SIZE*$id);$writen=fwrite($this->handle,pack('N',0));if($writen!==self::INT32_SIZE){throw
new\InvalidStateException("Cannot delete node number $id from journal.");}}}private
function
deleteAll(){if(!ftruncate($this->handle,self::HEADER_SIZE)){throw
new\InvalidStateException('Cannot truncate journal file.');}}private
function
lock(){if(!$this->handle){throw
new\InvalidStateException('File journal file is not opened');}if(!flock($this->handle,LOCK_EX)){throw
new\InvalidStateException('Cannot acquire exclusive lock on journal.');}if($this->lastModTime!==NULL){clearstatcache();if($this->lastModTime<@filemtime($this->file)){$this->nodeCache=$this->dataNodeFreeSpace=array();}}}private
function
...
new\DirectoryNotFoundException("Directory '$dir' not found.");}if(self::$useDirectories===NULL){$uniq=uniqid('_',TRUE);umask(0000);if(!@mkdir("$dir/$uniq",0777)){throw
...
new\NotSupportedException("PHP extension 'memcache' is not loaded.");}$this->prefix=$prefix;$this->journal=$journal;$this->memcache=new\Memcache;Nette\Debug::tryError();$this->memcache->connect($host,$port);if(Nette\Debug::catchError($e)){throw
new\InvalidStateException($e->getMessage());}}function
...
new\NotSupportedException('Dependent items are not supported by MemcachedStorage.');}$key=$this->prefix.$key;$meta=array(self::META_DATA=>$data);$expire=0;if(isset($dp[Cache::EXPIRATION])){$expire=(int)$dp[Cache::EXPIRATION];if(!empty($dp[Cache::SLIDING])){$meta[self::META_DELTA]=$expire;}}if(isset($dp[Cache::CALLBACKS])){$meta[self::META_CALLBACKS]=$dp[Cache::CALLBACKS];}if(isset($dp[Cache::TAGS])||isset($dp[Cache::PRIORITY])){if(!$this->journal){throw
new\InvalidStateException('CacheJournal has not been provided.');}$this->getJournal()->write($key,$dp);}$this->memcache->set($key,$meta,0,$expire);}function
remove($key){$this->memcache->delete($this->prefix.$key,0);}function
clean(array$conds){if(!empty($conds[Cache::ALL])){$this->memcache->flush();}elseif($this->journal){foreach($this->getJournal()->clean($conds)as$entry){$this->memcache->delete($entry,0);}}}}class
MemoryStorage
extends
Nette\Object
implements
...
clean(array$conds){if(!empty($conds[Cache::ALL])){$this->data=array();}}}}namespace Nette{use
Nette;class
...
static$extensions=array('ini'=>'Nette\Config\ConfigAdapterIni','neon'=>'Nette\Config\ConfigAdapterNeon');static
function
...
new\InvalidArgumentException("Unknown file extension '$file'.");}$data=call_user_func(array(self::$extensions[$extension],'load'),$file,$section);if($section){if(!isset($data[$section])||!is_array($data[$section])){throw
new\InvalidStateException("There is not section [$section] in '$file'.");}$data=$data[$section];}return
new
...
new\InvalidArgumentException("Unknown file extension '$file'.");}return
...
getIterator(){return
new
Nette\GenericRecursiveIterator(new\ArrayIterator($this));}function
...
new\FileNotFoundException("File '$file' is missing or is not readable.");}Nette\Debug::tryError();$ini=parse_ini_file($file,TRUE);if(Nette\Debug::catchError($e)){throw$e;}$separator=trim(self::$sectionSeparator);$data=array();foreach($ini
...
new\InvalidStateException("Invalid section [$secName] in '$file'.");}}}else{$cursor=&$data[$secName];}if(is_array($secData)&&is_array($cursor)){$secData=Nette\ArrayTools::mergeTree($secData,$cursor);}$cursor=$secData;}return$data;}static
function
...
new\InvalidStateException("Invalid section '$section'.");}$output[]="[$secName]";self::build($secData,$output,'');$output[]='';}if(!file_put_contents($file,implode(PHP_EOL,$output))){throw
...
new\InvalidArgumentException("The '$prefix$key' item must be scalar or array, ".gettype($val)." given.");}}}}use
Nette\Neon;final
class
ConfigAdapterNeon
implements
...
function
load($file){if(!is_file($file)||!is_readable($file)){throw
new\FileNotFoundException("File '$file' is missing or is not readable.");}$neon=Neon::decode(file_get_contents($file));$separator=trim(self::$sectionSeparator);$data=array();foreach($neon
...
new\IOException("Cannot write file '$file'.");}}}}namespace Nette\Database{use
Nette;use
...
table($table){return
new
Nette\Database\Selector\TableSelection($table,$this);}function
...
logQuery(Statement$result,array$params=NULL){if($this->disabled){return;}$source=NULL;foreach(debug_backtrace(FALSE)as$row){if(isset($row['file'])&&is_file($row['file'])&&strpos($row['file'],NETTE_DIR.DIRECTORY_SEPARATOR)!==0){$source=array($row['file'],(int)$row['line']);break;}}$this->totalTime+=$result->time;$this->queries[]=array($result->queryString,$params,$result->time,$result->rowCount(),$result->getConnection(),$source);}function
...
as$i=>$query){list($sql,$params,$time,$rows,$connection,$source)=$query;$explain=NULL;if($this->explain&&preg_match('#\s*SELECT\s#iA',$sql)){try{$explain=$connection->queryArgs('EXPLAIN '.$sql,$params)->fetchAll();}catch(\PDOException$e){}}$s.='<tr><td>'.sprintf('%0.3f',$time*1000);if($explain){$s.="<br /><a href='#' class='nette-toggler' rel='#nette-debug-database-row-{$h($this->name)}-$i'>explain&nbsp;&#x25ba;</a>";}$s.='</td><td class="database-sql">'.Connection::highlightSql(Nette\String::truncate($sql,self::$maxLength));if($explain){$s.="<table id='nette-debug-database-row-{$h($this->name)}-$i' class='nette-collapsed'><tr>";foreach($explain[0]as$col=>$foo){$s.="<th>{$h($col)}</th>";}$s.="</tr>";foreach($explain
as$row){$s.="<tr>";foreach($row
as$col){$s.="<td>{$h($col)}</td>";}$s.="</tr>";}$s.="</table>";}if($source){list($file,$line)=$source;$s.=(Nette\Debug::$editor?"<a href='{$h(Nette\DebugHelpers::editorLink($file,$line))}'":'<span')." class='database-source' title='{$h($file)}:$line'>"."{$h(basename(dirname($file)).'/'.basename($file))}:$line".(Nette\Debug::$editor?'</a>':'</span>');}$s.='</td><td>';foreach($params
as$param){$s.="{$h(Nette\String::truncate($param,self::$maxLength))}<br>";}$s.='</td><td>'.$rows.'</td></tr>';}return
...
as$s=>$val){if(preg_match("#$s#i",$type)){return$types[$type]=$val;}}}return$types[$type];}}}namespace Nette{use
Nette;class
ArrayHash
...
new\InvalidArgumentException("Key must be either a string or an integer, ".gettype($key)." given.");}$this->$key=$value;}function
offsetGet($key){return$this->$key;}function
offsetExists($key){return
isset($this->$key);}function
offsetUnset($key){unset($this->$key);}}}namespace Nette\Database{use
Nette;class
Row
extends
Nette\ArrayHash{function
__construct($statement){$statement->normalizeRow($this);}function
offsetGet($key){if(is_int($key)){$arr=array_values((array)$this);return$arr[$key];}return$this->$key;}}}namespace Nette\Database\Selector{use
Nette;class
TableSelection
extends
Nette\Object
...
instanceof
TableSelection){$clone=clone$parameters;if(!$clone->select){$clone->select=array($this->getPrimary($clone->name));}if($this->connection->getAttribute(\PDO::ATTR_DRIVER_NAME)!=='mysql'){$condition.=" IN ($clone)";}else{$in=array();foreach($clone
...
TableRow($row,$this);}$this->data=$this->rows;if(isset($row[$this->primary])&&!is_string($this->accessed)){$this->accessed[$this->primary]=TRUE;}}protected
function
...
insert($data){if($data
instanceof
TableSelection){$data=$data->getSql();}elseif($data
...
TableSelection($table,$this->connection);$referenced->where($table.'.'.$this->getPrimary($table),array_keys($keys));}else{$referenced=array();}}return$referenced;}function
getReferencingTable($table){$column=$this->connection->databaseReflection->getReferencingColumn($table,$this->name);$referencing=new
GroupedTableSelection($table,$this,$column);$referencing->where("$table.$column",array_keys((array)$this->rows));return$referencing;}private
function
...
extends
TableSelection{private$refTable;private$column;private$delimitedColumn;public$active;function
__construct($name,TableSelection$refTable,$column){parent::__construct($name,$refTable->connection);$this->refTable=$refTable;$this->through($column);}function
...
instanceof
TableSelection){$data=iterator_to_array($data);}if(is_array($data)){$data[$this->column]=$this->active;}return
...
__construct(array$data,TableSelection$table){$this->data=$data;$this->table=$table;}function
...
access($key,$delete=FALSE){if($this->table->connection->cache&&$this->table->access($key,$delete)){$this->data=$this->table[$this->data[$this->table->primary]]->data;}}}}namespace Nette\Database{use
Nette;class
...
process($sql,$params){$this->params=$params;$this->counter=0;$this->remaining=array();$cmd=strtoupper(substr(ltrim($sql),0,6));$this->arrayMode=$cmd==='INSERT'||$cmd==='REPLAC'?'values':'assoc';$sql=Nette\String::replace($sql,'~\'.*?\'|".*?"|:[a-zA-Z0-9_]+:|\?~s',array($this,'callback'));while($this->counter<count($params)){$sql.=' '.$this->formatValue($params[$this->counter++]);}return
...
Nette\ObjectMixin;use
Nette\Database\Reflection\DatabaseReflection;use
PDO;if(class_exists('PDO')){class
Statement
...
as$key=>$foo){$type=$this->getColumnMeta(count($this->types));if(isset($type['native_type'])){$this->types[$key]=DatabaseReflection::detectType($type['native_type']);}}}catch(\PDOException$e){}}foreach($this->types
...
ObjectMixin::has($this,$name);}function
__unset($name){throw
new\MemberAccessException("Cannot unset the property {$this->reflection->name}::\$$name.");}}}}namespace Nette{use
Nette;use
Nette\Environment;final
class
Debug{public
...
function
enable($mode=NULL,$logDirectory=NULL,$email=NULL){error_reporting(E_ALL|E_STRICT);if(is_bool($mode)){self::$productionMode=$mode;}elseif(is_string($mode)){$mode=preg_split('#[,\s]+#',"$mode 127.0.0.1 ::1");}if(is_array($mode)){self::$productionMode=!isset($_SERVER['REMOTE_ADDR'])||!in_array($_SERVER['REMOTE_ADDR'],$mode,TRUE);}if(self::$productionMode===self::DETECT){if(class_exists('Nette\Environment')){self::$productionMode=Environment::isProduction();}elseif(isset($_SERVER['SERVER_ADDR'])||isset($_SERVER['LOCAL_ADDR'])){$addrs=array();if(isset($_SERVER['HTTP_X_FORWARDED_FOR'])){$addrs=preg_split('#,\s*#',$_SERVER['HTTP_X_FORWARDED_FOR']);}if(isset($_SERVER['REMOTE_ADDR'])){$addrs[]=$_SERVER['REMOTE_ADDR'];}$addrs[]=isset($_SERVER['SERVER_ADDR'])?$_SERVER['SERVER_ADDR']:$_SERVER['LOCAL_ADDR'];self::$productionMode=FALSE;foreach($addrs
...
new\NotSupportedException('Function ini_set() must be enabled.');}if($email){if(!is_string($email)){throw
...
new\InvalidStateException('Logging directory is not specified in Nette\Debug::$logDirectory.');}elseif(!is_dir(self::$logDirectory)){throw
new\DirectoryNotFoundException("Directory '".self::$logDirectory."' is not found or is not directory.");}if($message
instanceof\Exception){$exception=$message;$message="PHP Fatal error: ".($message
instanceof\FatalErrorException?$exception->getMessage():"Uncaught exception ".get_class($exception)." with message '".$exception->getMessage()."'")." in ".$exception->getFile().":".$exception->getLine();$hash=md5($exception);$exceptionFilename="exception ".@date('Y-m-d H-i-s')." $hash.html";foreach(new\DirectoryIterator(self::$logDirectory)as$entry){if(strpos($entry,$hash)){$exceptionFilename=NULL;break;}}}error_log(@date('[Y-m-d H-i-s] ').trim($message).(self::$source?'  @  '.self::$source:'').(!empty($exceptionFilename)?'  @@  '.$exceptionFilename:'').PHP_EOL,3,self::$logDirectory.'/'.strtolower($priority).'.log');if(($priority===self::ERROR||$priority===self::CRITICAL)&&self::$email&&@filemtime(self::$logDirectory.'/email-sent')+self::$emailSnooze<time()&&@file_put_contents(self::$logDirectory.'/email-sent','sent')){call_user_func(self::$mailer,$message);}if(!empty($exceptionFilename)&&$logHandle=@fopen(self::$logDirectory.'/'.$exceptionFilename,'w')){ob_start();ob_start(function($buffer)use($logHandle){fwrite($logHandle,$buffer);},1);DebugHelpers::renderBlueScreen($exception);ob_end_flush();ob_end_clean();fclose($logHandle);}}static
function
_shutdownHandler(){static$types=array(E_ERROR=>1,E_CORE_ERROR=>1,E_COMPILE_ERROR=>1,E_PARSE=>1);$error=error_get_last();if(isset($types[$error['type']])){self::_exceptionHandler(new\FatalErrorException($error['message'],0,$error['type'],$error['file'],$error['line'],NULL));return;}if(self::$showBar&&!self::$productionMode&&!self::$ajaxDetected&&!self::$consoleMode&&(!preg_match('#^Content-Type: (?!text/html)#im',implode("\n",headers_list())))){DebugHelpers::renderDebugBar(self::$panels);}}static
function
...
function
barDump($var,$title=NULL){if(!self::$productionMode){$dump=array();foreach((is_array($var)?$var:array(''=>$var))as$key=>$val){$dump[$key]=DebugHelpers::clickableDump($val);}self::$panels[3]->data[]=array('title'=>$title,'dump'=>$dump);}return$var;}static
function
...
TRUE;}}Debug::_init();use
Nette\Config\Config;class
Configurator
extends
Object{public$defaultConfigFile='%appDir%/config.neon';public$defaultServices=array('Nette\\Application\\Application'=>array(__CLASS__,'createApplication'),'Nette\\Web\\HttpContext'=>'Nette\Web\HttpContext','Nette\\Web\\IHttpRequest'=>array(__CLASS__,'createHttpRequest'),'Nette\\Web\\IHttpResponse'=>'Nette\Web\HttpResponse','Nette\\Web\\IUser'=>'Nette\Web\User','Nette\\Caching\\ICacheStorage'=>array(__CLASS__,'createCacheStorage'),'Nette\\Caching\\ICacheJournal'=>array(__CLASS__,'createCacheJournal'),'Nette\\Mail\\IMailer'=>array(__CLASS__,'createMailer'),'Nette\\Web\\Session'=>'Nette\Web\Session','Nette\\Loaders\\RobotLoader'=>array(__CLASS__,'createRobotLoader'));function
...
new\InvalidStateException("Factory method is not specified for service $key.");}if($value->run){$runServices[]=$key;}}}}if(!$config->php){$config->php=$config->set;unset($config->set);}if($config->php
instanceof
...
Environment::getContext();$context->addService('Nette\\Application\\IRouter','Nette\Application\MultiRouter');if(!$context->hasService('Nette\\Application\\IPresenterFactory')){$context->addService('Nette\\Application\\IPresenterFactory',function()use($context){return
new
...
Nette\Web\HttpRequestFactory;$factory->setEncoding('UTF-8');return$factory->createHttpRequest();}static
function
createCacheStorage(){$dir=Environment::getVariable('tempDir').'/cache';umask(0000);@mkdir($dir,0777);return
new
Nette\Caching\FileStorage($dir,Environment::getService('Nette\\Caching\\ICacheJournal'));}static
function
createCacheJournal(){return
new
Nette\Caching\FileJournal(Environment::getVariable('tempDir'));}static
function
createMailer(array$options=NULL){if(isset($options['smtp'])){return
new
Nette\Mail\SmtpMailer($options);}else{return
new
Nette\Mail\SendmailMailer;}}static
function
...
new\InvalidStateException('Environment name has already been set.');}}static
function
...
new\InvalidStateException("Unknown environment variable '$name'.");}}}static
function
...
new\InvalidStateException("Circular reference detected for variables: ".implode(', ',array_keys($livelock)).".");}try{$livelock[$var]=TRUE;$val=Environment::getVariable($var);unset($livelock[$var]);}catch(\Exception$e){$livelock=array();throw$e;}if(!is_scalar($val)){throw
new\InvalidStateException("Environment variable '$var' is not scalar.");}return$val;},$var);}return$var;}static
function
...
getForm($need=TRUE){return$this->lookup('Nette\Forms\Form',$need);}function
getHtmlName(){if($this->htmlName===NULL){$s='';$name=$this->getName();$obj=$this->lookup('Nette\Forms\FormContainer',TRUE);while(!$obj
instanceof
Form){$s="[$name]$s";$name=$obj->getName();$obj=$obj->lookup('Nette\Forms\FormContainer',TRUE);}$name.=$s;if(is_numeric($name)||in_array($name,array('attributes','children','elements','focus','length','reset','style','submit','onsubmit'))){$name.='_';}$this->htmlName=$name;}return$this->htmlName;}function
...
getControl(){$control=parent::getControl();if($this->skipFirst){reset($this->items);$control->data('nette-empty-value',$this->useKeys?key($this->items):current($this->items));}$selected=$this->getValue();$selected=is_array($selected)?array_flip($selected):array($selected=>TRUE);$option=Nette\Web\Html::el('option');foreach($this->items
...
instanceof
Nette\Web\Html){$dest->add((string)$value2->selected(isset($selected[$key2])));}else{$key2=$this->useKeys?$key2:$value2;$value2=$this->translate((string)$value2);$dest->add((string)$option->value($key2===$value2?NULL:$key2)->selected(isset($selected[$key2]))->setText($value2));}}}return$control;}}class
MultiSelectBox
extends
SelectBox{function
...
sanitize($value){if($this->control->maxlength&&Nette\String::length($value)>$this->control->maxlength){$value=iconv_substr($value,0,$this->control->maxlength,'UTF-8');}return
Nette\String::trim(strtr($value,"\r\n",'  '));}function
...
Uri($this->headers['referer']):NULL;}function
...
UriScript;$uri->scheme=isset($_SERVER['HTTPS'])&&strcasecmp($_SERVER['HTTPS'],'off')?'https':'http';$uri->user=isset($_SERVER['PHP_AUTH_USER'])?$_SERVER['PHP_AUTH_USER']:'';$uri->password=isset($_SERVER['PHP_AUTH_PW'])?$_SERVER['PHP_AUTH_PW']:'';if(isset($_SERVER['HTTP_HOST'])){$pair=explode(':',$_SERVER['HTTP_HOST']);}elseif(isset($_SERVER['SERVER_NAME'])){$pair=explode(':',$_SERVER['SERVER_NAME']);}else{$pair=array('');}$uri->host=preg_match('#^[-._a-z0-9]+$#',$pair[0])?$pair[0]:'';if(isset($pair[1])){$uri->port=(int)$pair[1];}elseif(isset($_SERVER['SERVER_PORT'])){$uri->port=(int)$_SERVER['SERVER_PORT'];}if(isset($_SERVER['REQUEST_URI'])){$requestUri=$_SERVER['REQUEST_URI'];}elseif(isset($_SERVER['ORIG_PATH_INFO'])){$requestUri=$_SERVER['ORIG_PATH_INFO'];if(isset($_SERVER['QUERY_STRING'])&&$_SERVER['QUERY_STRING']!=''){$requestUri.='?'.$_SERVER['QUERY_STRING'];}}else{$requestUri='';}$requestUri=String::replace($requestUri,$this->uriFilters['uri']);$tmp=explode('?',$requestUri,2);$uri->path=String::replace($tmp[0],$this->uriFilters['path']);$uri->query=isset($tmp[1])?$tmp[1]:'';$uri->canonicalize();$uri->path=String::fixEncoding($uri->path);if(isset($_SERVER['DOCUMENT_ROOT'],$_SERVER['SCRIPT_FILENAME'])&&strncmp($_SERVER['DOCUMENT_ROOT'],$_SERVER['SCRIPT_FILENAME'],strlen($_SERVER['DOCUMENT_ROOT']))===0){$script='/'.ltrim(strtr(substr($_SERVER['SCRIPT_FILENAME'],strlen($_SERVER['DOCUMENT_ROOT'])),'\\','/'),'/');}elseif(isset($_SERVER['SCRIPT_NAME'])){$script=$_SERVER['SCRIPT_NAME'];}else{$script='/';}if(strncasecmp($uri->path.'/',$script.'/',strlen($script)+1)===0){$uri->scriptPath=substr($uri->path,0,strlen($script));}elseif(strncasecmp($uri->path,$script,strrpos($script,'/')+1)===0){$uri->scriptPath=substr($uri->path,0,strrpos($script,'/')+1);}else{$uri->scriptPath='/';}$useFilter=(!in_array(ini_get('filter.default'),array('','unsafe_raw'))||ini_get('filter.default_flags'));parse_str($uri->query,$query);if(!$query){$query=$useFilter?filter_input_array(INPUT_GET,FILTER_UNSAFE_RAW):(empty($_GET)?array():$_GET);}$post=$useFilter?filter_input_array(INPUT_POST,FILTER_UNSAFE_RAW):(empty($_POST)?array():$_POST);$cookies=$useFilter?filter_input_array(INPUT_COOKIE,FILTER_UNSAFE_RAW):(empty($_COOKIE)?array():$_COOKIE);$gpc=(bool)get_magic_quotes_gpc();$old=error_reporting(error_reporting()^E_NOTICE);if($gpc||$this->encoding){$utf=strcasecmp($this->encoding,'UTF-8')===0;$list=array(&$query,&$post,&$cookies);while(list($key,$val)=each($list)){foreach($val
as$k=>$v){unset($list[$key][$k]);if($gpc){$k=stripslashes($k);}if($this->encoding&&is_string($k)&&(preg_match(self::NONCHARS,$k)||preg_last_error())){}elseif(is_array($v)){$list[$key][$k]=$v;$list[]=&$list[$key][$k];}else{if($gpc&&!$useFilter){$v=stripSlashes($v);}if($this->encoding){if($utf){$v=String::fixEncoding($v);}else{if(!String::checkEncoding($v)){$v=iconv($this->encoding,'UTF-8//IGNORE',$v);}$v=html_entity_decode($v,ENT_QUOTES,'UTF-8');}$v=preg_replace(self::NONCHARS,'',$v);}$list[$key][$k]=$v;}}}unset($list,$key,$val,$k,$v);}$files=array();$list=array();if(!empty($_FILES)){foreach($_FILES
as$k=>$v){if($this->encoding&&is_string($k)&&(preg_match(self::NONCHARS,$k)||preg_last_error()))continue;$v['@']=&$files[$k];$list[]=$v;}}while(list(,$v)=each($list)){if(!isset($v['name'])){continue;}elseif(!is_array($v['name'])){if($gpc){$v['name']=stripSlashes($v['name']);}if($this->encoding){$v['name']=preg_replace(self::NONCHARS,'',String::fixEncoding($v['name']));}$v['@']=new
HttpUploadedFile($v);continue;}foreach($v['name']as$k=>$foo){if($this->encoding&&is_string($k)&&(preg_match(self::NONCHARS,$k)||preg_last_error()))continue;$list[]=array('name'=>$v['name'][$k],'type'=>$v['type'][$k],'size'=>$v['size'][$k],'tmp_name'=>$v['tmp_name'][$k],'error'=>$v['error'][$k],'@'=>&$v['@'][$k]);}}error_reporting($old);if(function_exists('apache_request_headers')){$headers=array_change_key_case(apache_request_headers(),CASE_LOWER);}else{$headers=array();foreach($_SERVER
as$k=>$v){if(strncmp($k,'HTTP_',5)==0){$k=substr($k,5);}elseif(strncmp($k,'CONTENT_',8)){continue;}$headers[strtr(strtolower($k),'_','-')]=$v;}}return
new
HttpRequest($uri,$query,$post,$files,$cookies,$headers,isset($_SERVER['REQUEST_METHOD'])?$_SERVER['REQUEST_METHOD']:NULL,isset($_SERVER['REMOTE_ADDR'])?$_SERVER['REMOTE_ADDR']:NULL,isset($_SERVER['REMOTE_HOST'])?$_SERVER['REMOTE_HOST']:NULL);}}final
class
HttpResponse
extends
Nette\Object
...
getContentType(){if($this->isOk()&&$this->type===NULL){$this->type=Nette\Tools::detectMimeType($this->tmpName);}return$this->type;}function
...
new\InvalidStateException('A session had already been started by session.auto-start or session_start().');}$this->configure($this->options);Nette\Debug::tryError();session_start();if(Nette\Debug::catchError($e)){@session_write_close();throw
new\InvalidStateException($e->getMessage());}self::$started=TRUE;if($this->regenerationNeeded){session_regenerate_id(TRUE);$this->regenerationNeeded=FALSE;}unset($_SESSION['__NT'],$_SESSION['__NS'],$_SESSION['__NM']);$nf=&$_SESSION['__NF'];if(empty($nf)){$nf=array('C'=>0);}else{$nf['C']++;}$browserKey=$this->getHttpRequest()->getCookie('nette-browser');if(!$browserKey){$browserKey=Nette\String::random();}$browserClosed=!isset($nf['B'])||$nf['B']!==$browserKey;$nf['B']=$browserKey;$this->sendCookie();if(isset($nf['META'])){$now=time();foreach($nf['META']as$namespace=>$metadata){if(is_array($metadata)){foreach($metadata
...
new\InvalidStateException("Unable to set '$key' when session has been started.");}$key="session_$key";$key($value);}elseif(strncmp($key,'cookie_',7)===0){if(!isset($cookie)){$cookie=session_get_cookie_params();}$cookie[substr($key,7)]=$value;}elseif(!function_exists('ini_set')){if(ini_get($key)!=$value&&!Nette\Framework::$iAmUsingBadHost){throw
...
setExpiration($time){if(empty($time)){return$this->setOptions(array('gc_maxlifetime'=>self::DEFAULT_FILE_LIFETIME,'cookie_lifetime'=>0));}else{$time=Nette\Tools::createDateTime($time)->format('U')-time();return$this->setOptions(array('gc_maxlifetime'=>$time,'cookie_lifetime'=>$time));}}function
...
new\InvalidStateException("Unable to set storage when session has been started.");}session_set_save_handler(array($storage,'open'),array($storage,'close'),array($storage,'read'),array($storage,'write'),array($storage,'remove'),array($storage,'clean'));}private
function
...
new\InvalidStateException('Authentication handler has not been set.');}$this->logout(TRUE);$credentials=func_get_args();$this->setIdentity($handler->authenticate($credentials));$this->setAuthenticated(TRUE);$this->onLoggedIn($this);}final
function
...
function
getSession(){return
Environment::getSession();}}}namespace Nette{use
Nette;class
...
new\InvalidArgumentException("Service named '$name' is empty.");}$this->factories[$lower]=array($service,$singleton,$options);$this->registry[$lower]=&$this->factories[$lower][3];}return$this;}function
...
new\InvalidStateException("Unable to set options, method $factory::setOptions() is missing.");}}}else{$factory=callback($factory);if(!$factory->isCallable()){throw
...
isset($this->registry[$lower])||(!$created&&isset($this->factories[$lower]));}}}namespace Nette\Templates{use
Nette;use
Nette\Environment;use
...
function
getCache(){return
Environment::getCache('Nette.Template.Cache');}}class
TemplateException
extends\InvalidStateException
implements
Nette\IDebugPanel{public$sourceFile;public$sourceLine;function
...
getPanel(){$link=Nette\DebugHelpers::editorLink($this->sourceFile,$this->sourceLine);return'<p><b>File:</b> '.($link?'<a href="'.htmlspecialchars($link).'">':'').htmlspecialchars($this->sourceFile).($link?'</a>':'').'&nbsp; <b>Line:</b> '.($this->sourceLine?$this->sourceLine:'n/a').'</p>'.($this->sourceLine?'<pre>'.Nette\DebugHelpers::highlightFile($this->sourceFile,$this->sourceLine).'</pre>':'');}function
getId(){}}class
LatteException
extends
TemplateException{}use
Nette\String;class
LatteFilter
extends
Nette\Object{const
...
__invoke($s){if(!String::checkEncoding($s)){throw
new
LatteException('Template is not valid UTF-8 stream.');}if(!$this->macroRe){$this->setDelimiters('\\{(?![\\s\'"{}*])','\\}');}$this->context=LatteFilter::CONTEXT_NONE;$this->escape='$template->escape';$this->getHandler()->initialize($this,$s);$s=$this->parse("\n".$s);$this->getHandler()->finalize($s);return$s;}private
function
...
new
LatteException("Unknown macro {{$matches['macro']}}",0,$this->line);}$nl=isset($matches['newline'])?"\n":'';if($nl&&$matches['indent']&&strncmp($code,'<?php echo ',11)){$this->output.="\n".$code;}else{$this->output.=$matches['indent'].$code.(substr($code,-2)==='?>'?$nl:'');}}else{$this->output.=$matches[0];}}foreach($this->tags
as$tag){if(!$tag->isMacro&&!empty($tag->attrs)){throw
new
LatteException("Missing end tag </$tag->name> for macro-attribute ".self::HTML_PREFIX.implode(' and '.self::HTML_PREFIX,array_keys($tag->attrs)).".",0,$this->line);}}return$this->output.substr($this->input,$this->offset);}private
function
contextText(){$matches=$this->match('~
			(?:\n[ \t]*)?<(?P<closing>/?)(?P<tag>[a-z0-9:]+)|  ##  begin of HTML tag <tag </tag - ignores <!DOCTYPE
			<(?P<htmlcomment>!--)|           ##  begin of HTML comment <!--
			'.$this->macroRe.'           ##  curly tag
		~xsi');if(!$matches||!empty($matches['macro'])||!empty($matches['comment'])){}elseif(!empty($matches['htmlcomment'])){$this->context=self::CONTEXT_COMMENT;$this->escape='Nette\Templates\TemplateHelpers::escapeHtmlComment';}elseif(empty($matches['closing'])){$tag=$this->tags[]=(object)NULL;$tag->name=$matches['tag'];$tag->closing=FALSE;$tag->isMacro=String::startsWith($tag->name,self::HTML_PREFIX);$tag->attrs=array();$tag->pos=strlen($this->output);$this->context=self::CONTEXT_TAG;$this->escape='Nette\Templates\TemplateHelpers::escapeHtml';}else{do{$tag=array_pop($this->tags);if(!$tag){$tag=(object)NULL;$tag->name=$matches['tag'];$tag->isMacro=String::startsWith($tag->name,self::HTML_PREFIX);}}while(strcasecmp($tag->name,$matches['tag']));$this->tags[]=$tag;$tag->closing=TRUE;$tag->pos=strlen($this->output);$this->context=self::CONTEXT_TAG;$this->escape='Nette\Templates\TemplateHelpers::escapeHtml';}return$matches;}private
function
contextCData(){$tag=end($this->tags);$matches=$this->match('~
			</'.$tag->name.'(?![a-z0-9:])| ##  end HTML tag </tag
			'.$this->macroRe.'           ##  curly tag
		~xsi');if($matches&&empty($matches['macro'])&&empty($matches['comment'])){$tag->closing=TRUE;$tag->pos=strlen($this->output);$this->context=self::CONTEXT_TAG;$this->escape='Nette\Templates\TemplateHelpers::escapeHtml';}return$matches;}private
function
contextTag(){$matches=$this->match('~
			(?P<end>\ ?/?>)(?P<tagnewline>[\ \t]*(?=\r|\n))?|  ##  end of HTML tag
			'.$this->macroRe.'|          ##  curly tag
			\s*(?P<attr>[^\s/>={]+)(?:\s*=\s*(?P<value>["\']|[^\s/>{]+))? ## begin of HTML attribute
		~xsi');if(!$matches||!empty($matches['macro'])||!empty($matches['comment'])){}elseif(!empty($matches['end'])){$tag=end($this->tags);$isEmpty=!$tag->closing&&(strpos($matches['end'],'/')!==FALSE||isset(Nette\Web\Html::$emptyElements[strtolower($tag->name)]));if($isEmpty){$matches[0]=(Nette\Web\Html::$xhtml?' />':'>').(isset($matches['tagnewline'])?$matches['tagnewline']:'');}if($tag->isMacro||!empty($tag->attrs)){if($tag->isMacro){$code=$this->handler->tagMacro(substr($tag->name,strlen(self::HTML_PREFIX)),$tag->attrs,$tag->closing);if($code===FALSE){throw
new
LatteException("Unknown tag-macro <$tag->name>",0,$this->line);}if($isEmpty){$code.=$this->handler->tagMacro(substr($tag->name,strlen(self::HTML_PREFIX)),$tag->attrs,TRUE);}}else{$code=substr($this->output,$tag->pos).$matches[0].(isset($matches['tagnewline'])?"\n":'');$code=$this->handler->attrsMacro($code,$tag->attrs,$tag->closing);if($code===FALSE){throw
new
LatteException("Unknown macro-attribute ".self::HTML_PREFIX.implode(' or '.self::HTML_PREFIX,array_keys($tag->attrs)),0,$this->line);}if($isEmpty){$code=$this->handler->attrsMacro($code,$tag->attrs,TRUE);}}$this->output=substr_replace($this->output,$code,$tag->pos);$matches[0]='';}if($isEmpty){$tag->closing=TRUE;}if(!$tag->closing&&(strcasecmp($tag->name,'script')===0||strcasecmp($tag->name,'style')===0)){$this->context=self::CONTEXT_CDATA;$this->escape=strcasecmp($tag->name,'style')?'Nette\Templates\TemplateHelpers::escapeJs':'Nette\Templates\TemplateHelpers::escapeCss';}else{$this->context=self::CONTEXT_TEXT;$this->escape='Nette\Templates\TemplateHelpers::escapeHtml';if($tag->closing)array_pop($this->tags);}}else{$name=$matches['attr'];$value=isset($matches['value'])?$matches['value']:'';if($isSpecial=String::startsWith($name,self::HTML_PREFIX)){$name=substr($name,strlen(self::HTML_PREFIX));}$tag=end($this->tags);if($isSpecial||$tag->isMacro){if($value==='"'||$value==="'"){if($matches=$this->match('~(.*?)'.$value.'~xsi')){$value=$matches[1];}}$tag->attrs[$name]=$value;$matches[0]='';}elseif($value==='"'||$value==="'"){$this->context=self::CONTEXT_ATTRIBUTE;$this->quote=$value;$this->escape=strncasecmp($name,'on',2)?(strcasecmp($name,'style')?'Nette\Templates\TemplateHelpers::escapeHtml':'Nette\Templates\TemplateHelpers::escapeHtmlCss'):'Nette\Templates\TemplateHelpers::escapeHtmlJs';}}return$matches;}private
function
contextAttribute(){$matches=$this->match('~
			('.$this->quote.')|      ##  1) end of HTML attribute
			'.$this->macroRe.'           ##  curly tag
		~xsi');if($matches&&empty($matches['macro'])&&empty($matches['comment'])){$this->context=self::CONTEXT_TAG;$this->escape='Nette\Templates\TemplateHelpers::escapeHtml';}return$matches;}private
function
contextComment(){$matches=$this->match('~
			(--\s*>)|                    ##  1) end of HTML comment
			'.$this->macroRe.'           ##  curly tag
		~xsi');if($matches&&empty($matches['macro'])&&empty($matches['comment'])){$this->context=self::CONTEXT_TEXT;$this->escape='Nette\Templates\TemplateHelpers::escapeHtml';}return$matches;}private
function
...
Nette\Tokenizer(array(self::T_WHITESPACE=>'\s+',self::T_COMMENT=>'(?s)/\*.*?\*/',LatteFilter::RE_STRING,'(?:true|false|null|and|or|xor|clone|new|instanceof|return|continue|break|[A-Z_][A-Z0-9_]{2,})(?!\w)','\([a-z]+\)',self::T_VARIABLE=>'\$\w+',self::T_NUMBER=>'[+-]?[0-9]+(?:\.[0-9]+)?(?:e[0-9]+)?',self::T_SYMBOL=>'\w+(?:-\w+)*','::|=>|[^"\']'),'u');}function
initialize($filter,&$s){$this->filter=$filter;$this->nodes=array();$this->blocks=array();$this->namedBlocks=array();$this->extends=NULL;$this->uniq=String::random();$this->cacheCounter=0;$filter->context=LatteFilter::CONTEXT_TEXT;$filter->escape='Nette\Templates\TemplateHelpers::escapeHtml';}function
finalize(&$s){if(count($this->blocks)===1){$s.=$this->macro('/block');}elseif($this->blocks){throw
new
LatteException("There are unclosed blocks.",0,$this->filter->line);}if($this->namedBlocks||$this->extends){$s='<?php
if ($_l->extends) {
	ob_start();
...
FALSE;}$closing=$macro[0]==='/';if($closing){$node=array_pop($this->nodes);if(!$node||"/$node->name"!==$macro||($content&&!String::startsWith("$node->content ","$content "))||$modifiers){$macro.=$content?' ':'';throw
new
LatteException("Unexpected macro {{$macro}{$content}{$modifiers}}".($node?", expecting {/$node->name}".($content&&$node->content?" or eventually {/$node->name $node->content}":''):''),0,$this->filter->line);}$node->content=$node->modifiers='';}else{$node=(object)NULL;$node->name=$macro;$node->content=$content;$node->modifiers=$modifiers;if(isset($this->macros["/$macro"])){$this->nodes[]=$node;}}$This=$this;return
String::replace($this->macros[$macro],'#%(.*?)%#',function($m)use($This,$node){if($m[1]){return
...
new
LatteException("Unknown syntax '$var'",0,$this->filter->line);}}function
...
new
LatteException("Missing destination in {include}",0,$this->filter->line);}elseif($destination[0]==='#'){$destination=ltrim($destination,'#');if(!String::match($destination,'#^'.self::RE_IDENTIFIER.'$#')){throw
new
LatteException("Included block name must be alphanumeric string, '$destination' given.",0,$this->filter->line);}$parent=$destination==='parent';if($destination==='parent'||$destination==='this'){$item=end($this->blocks);while($item&&$item[0]!==self::BLOCK_NAMED)$item=prev($this->blocks);if(!$item){throw
new
LatteException("Cannot include $destination block outside of any block.",0,$this->filter->line);}$destination=$item[1];}$name=var_export($destination,TRUE);$params.=$isDefinition?'get_defined_vars()':'$template->getParams()';$cmd=isset($this->namedBlocks[$destination])&&!$parent?"call_user_func(reset(\$_l->blocks[$name]), \$_l, $params)":'Nette\Templates\LatteMacros::callBlock'.($parent?'Parent':'')."(\$_l, $name, $params)";return$modifiers?"ob_start(); $cmd; echo ".$this->formatModifiers('ob_get_clean()',$modifiers):$cmd;}else{$destination=$this->formatString($destination);$cmd='Nette\Templates\LatteMacros::includeTemplate('.$destination.', '.$params.'$template->getParams(), $_l->templates['.var_export($this->uniq,TRUE).'])';return$modifiers?'echo '.$this->formatModifiers($cmd.'->__toString(TRUE)',$modifiers):$cmd.'->render()';}}function
macroExtends($content){if(!$content){throw
new
LatteException("Missing destination in {extends}",0,$this->filter->line);}if(!empty($this->blocks)){throw
new
LatteException("{extends} must be placed outside any block.",0,$this->filter->line);}if($this->extends!==NULL){throw
new
LatteException("Multiple {extends} declarations are not allowed.",0,$this->filter->line);}$this->extends=$content!=='none';return$this->extends?'$_l->extends = '.($content==='auto'?'$layout':$this->formatMacroArgs($content)):'';}function
...
new
LatteException("Invalid capture block parameter '$name'",0,$this->filter->line);}$this->blocks[]=array(self::BLOCK_CAPTURE,$name,$modifiers);return'ob_start()';}function
...
macroCache($content){return'if (Nette\Templates\CachingHelper::create('.var_export($this->uniq.':'.$this->cacheCounter++,TRUE).', $_l->g->caches'.$this->formatArray($content,', ').')) {';}function
macroForeach($content){return'$iterator = $_l->its[] = new Nette\SmartCachingIterator('.preg_replace('#(.*)\s+as\s+#i','$1) as ',$this->formatMacroArgs($content),1);}function
...
macroContentType($content){if(strpos($content,'html')!==FALSE){$this->filter->escape='Nette\Templates\TemplateHelpers::escapeHtml';$this->filter->context=LatteFilter::CONTEXT_TEXT;}elseif(strpos($content,'xml')!==FALSE){$this->filter->escape='Nette\Templates\TemplateHelpers::escapeXml';$this->filter->context=LatteFilter::CONTEXT_NONE;}elseif(strpos($content,'javascript')!==FALSE){$this->filter->escape='Nette\Templates\TemplateHelpers::escapeJs';$this->filter->context=LatteFilter::CONTEXT_NONE;}elseif(strpos($content,'css')!==FALSE){$this->filter->escape='Nette\Templates\TemplateHelpers::escapeCss';$this->filter->context=LatteFilter::CONTEXT_NONE;}elseif(strpos($content,'plain')!==FALSE){$this->filter->escape='';$this->filter->context=LatteFilter::CONTEXT_NONE;}else{$this->filter->escape='$template->escape';$this->filter->context=LatteFilter::CONTEXT_NONE;}return
strpos($content,'/')?'Nette\Environment::getHttpResponse()->setHeader("Content-Type", "'.$content.'")':'';}function
macroDump($content){return'Nette\Debug::barDump('.($content?'array('.var_export($this->formatMacroArgs($content),TRUE)." => $content)":'get_defined_vars()').', "Template " . str_replace(dirname(dirname($template->getFile())), "\xE2\x80\xA6", $template->getFile()))';}function
...
new
LatteException("Missing control name in {control}",0,$this->filter->line);}$pair=explode(':',$pair,2);$name=$this->formatString($pair[0]);$method=isset($pair[1])?ucfirst($pair[1]):'';$method=String::match($method,'#^('.self::RE_IDENTIFIER.'|)$#')?"render$method":"{\"render$method\"}";$param=$this->formatArray($content);if(strpos($content,'=>')===FALSE)$param=substr($param,6,-1);return($name[0]==='$'?"if (is_object($name)) \$_ctrl = $name; else ":'').'$_ctrl = $control->getWidget('.$name.'); '.'if ($_ctrl instanceof Nette\Application\IPartiallyRenderable) $_ctrl->validateControl(); '."\$_ctrl->$method($param)";}function
...
macroIfCurrent($content){return($content?'try { $presenter->link('.$this->formatLink($content).'); } catch (Nette\Application\InvalidLinkException $e) {}':'').'; if ($presenter->getLastCreatedRequestFlag("current")):';}private
function
...
new
LatteException("Modifier name must be alphanumeric string, '$token' given.",0,$this->filter->line);}}else{if($token===':'||$token===','){$var=$var.', ';}elseif($token==='|'){$var=$var.')';$inside=FALSE;}else{$var.=$token;}}}return$inside?"$var)":$var;}function
fetchToken(&$s){if($matches=String::match($s,'#^((?>'.LatteFilter::RE_STRING.'|[^\'"\s,]+)+)\s*,?\s*(.*)$#s')){$s=$matches[2];return$matches[1];}return
NULL;}function
...
new\InvalidStateException("Cannot include undefined block '$name'.");}$block=reset($context->blocks[$name]);$block($context,$params);}static
function
callBlockParent($context,$name,$params){if(empty($context->blocks[$name])||($block=next($context->blocks[$name]))===FALSE){throw
new\InvalidStateException("Cannot include undefined parent block '$name'.");}$block($context,$params);}static
function
...
instanceof
Nette\Application\Control){foreach($control->getComponents(FALSE,'Nette\Application\Control')as$child){if($child->isControlInvalid()){$child->render();}}}}}}namespace Nette\Loaders{use
Nette;use
Nette\String;use
Nette\Caching\Cache;class
RobotLoader
...
function
addClass($class,$file,$time){$lClass=strtolower($class);if(isset($this->list[$lClass][0])&&($file2=$this->list[$lClass][0])!==$file&&is_file($file2)){if($this->files[$file2]!==filemtime($file2)){$this->scanScript($file2);return$this->addClass($class,$file,$time);}$e=new\InvalidStateException("Ambiguous class '$class' resolution; defined in $file and in ".$this->list[$lClass][0].".");{throw$e;}}$this->list[$lClass]=array($file,$time,$class);$this->files[$file]=$time;}private
function
...
as$class=>$pair){if($pair&&$pair[0]===$file)unset($this->list[$class]);}if($matches=String::match($s,'#//nette'.'loader=(\S*)#')){foreach(explode(',',$matches[1])as$name){$this->addClass($name,$file,$time);}return;}foreach(token_get_all($s)as$token){if(is_array($token)){switch($token[0]){case
T_COMMENT:case
T_DOC_COMMENT:case
...
setCacheStorage(Nette\Caching\ICacheStorage$storage){$this->cacheStorage=$storage;return$this;}function
...
Nette\Caching\DummyStorage;}return
new
...
as$email=>$name){if($name!==NULL&&!Nette\String::checkEncoding($name)){throw
...
new\InvalidArgumentException("Name must not contain line separator.");}$tmp[$email]=$name;}}else{$value=(string)$value;if(!Nette\String::checkEncoding($value)){throw
...
generateMessage(){$output='';$boundary='--------'.Nette\String::random();foreach($this->headers
...
addEmbeddedFile($file,$content=NULL,$contentType=NULL){$hostname=isset($_SERVER['HTTP_HOST'])?$_SERVER['HTTP_HOST']:(isset($_SERVER['SERVER_NAME'])?$_SERVER['SERVER_NAME']:'localhost');return$this->inlines[$file]=$this->createAttachment($file,$content,$contentType,'inline')->setHeader('Content-ID','<'.String::random()."@$hostname>");}function
...
MailMimePart;if($content===NULL){$content=file_get_contents($file);if($content===FALSE){throw
new\FileNotFoundException("Unable to read file '$file'.");}}else{$content=(string)$content;}$part->setBody($content);$part->setContentType($contentType?$contentType:Nette\Tools::detectMimeTypeFromString($content));$part->setEncoding(preg_match('#(multipart|message)/#A',$contentType)?self::ENCODING_8BIT:self::ENCODING_BASE64);$part->setHeader('Content-Disposition',$disposition.'; filename="'.String::fixEncoding(basename($file)).'"');return$part;}function
...
function
build(){$mail=clone$this;$hostname=isset($_SERVER['HTTP_HOST'])?$_SERVER['HTTP_HOST']:(isset($_SERVER['SERVER_NAME'])?$_SERVER['SERVER_NAME']:'localhost');$mail->setHeader('Message-ID','<'.String::random()."@$hostname>");$mail->buildHtml();$mail->buildText();$cursor=$mail;if($mail->attachments){$tmp=$cursor->setContentType('multipart/mixed');$cursor=$cursor->addPart();foreach($mail->attachments
...
function
read(){$s='';while(($line=fgets($this->connection,1e3))!=NULL){$s.=$line;if(substr($line,3,1)===' '){break;}}return$s;}}}namespace Nette\Reflection{use
Nette;class
Annotation
extends
Nette\Object
...
getClassName(){return($tmp=Nette\String::match($this,'#>\s+([a-z0-9_\\\\]+)#i'))?$tmp[1]:NULL;}function
...
new
AuthenticationException("User '$username' not found.",self::IDENTITY_NOT_FOUND);}}}namespace Nette\Templates{use
Nette;abstract
class
Template
extends
...
render(){throw
new\NotImplementedException;}function
save($file){if(file_put_contents($file,$this->__toString(TRUE))===FALSE){throw
new\IOException("Unable to save file '$file'.");}}function
__toString(){ob_start();try{$this->render();return
ob_get_clean();}catch(\Exception$e){ob_end_clean();if(func_num_args()&&func_get_arg(0)){throw$e;}else{Nette\Debug::toStringException($e);}}}function
...
new\InvalidStateException("The variable '$name' already exists.");}$this->params[$name]=$value;}function
...
Nette\Loaders\LimitedScope;class
FileTemplate
extends
Template
implements
...
new\FileNotFoundException("Missing template file '$file'.");}return$this;}function
...
instanceof
TemplateCacheStorage){$storage->hint=str_replace(dirname(dirname($this->file)),'',$this->file);}$cached=$content=$cache[$this->file];if($content===NULL){try{$content=$this->compile(file_get_contents($this->file));$content="<?php\n\n// source file: $this->file\n\n?>$content";}catch(TemplateException$e){$e->setSourceFile($this->file);throw$e;}$cache->save($this->file,$content,array(Cache::FILES=>$this->file,Cache::CONSTS=>'Nette\Framework::REVISION'));$cache->release();$cached=$cache[$this->file];}if($cached!==NULL&&$storage
instanceof
TemplateCacheStorage){LimitedScope::load($cached['file'],$this->getParams());flock($cached['handle'],LOCK_UN);fclose($cached['handle']);}else{LimitedScope::evaluate($content,$this->getParams());}}function
setCacheStorage(Nette\Caching\ICacheStorage$storage){$this->cacheStorage=$storage;}function
getCacheStorage(){if($this->cacheStorage===NULL){$dir=Environment::getVariable('tempDir').'/cache';umask(0000);@mkdir($dir,0777);$this->cacheStorage=new
TemplateCacheStorage($dir);}return$this->cacheStorage;}}class
TemplateCacheStorage
extends
Nette\Caching\FileStorage{public$hint;protected
function
readData($meta){return
array('file'=>$meta[self::FILE],'handle'=>$meta[self::HANDLE]);}protected
function
getCacheFile($key){$key=substr_replace($key,trim(strtr($this->hint,'\\/@','.._'),'.').'-',strpos($key,Nette\Caching\Cache::NAMESPACE_SEPARATOR)+1,0);return
parent::getCacheFile($key).'.php';}}use
Nette\String;final
class
...
String::replace($s,'#(src|href|action)\s*=\s*(["\'])(?![a-z]+:|[\x01/\\#])#','$1=$2<?php echo \\$basePath ?>/');}static
function
netteLinks($s){return
...
str_replace(']]>',']]\x3E',Nette\Json::encode($s));}static
function
...
NULL;}if(!isset($format)){$format=self::$dateFormat;}$time=Nette\Tools::createDateTime($time);return
...
function
dataStream($data,$type=NULL){if($type===NULL){$type=Nette\Tools::detectMimeTypeFromString($data,NULL);}return'data:'.($type?"$type;":'').'base64,'.base64_encode($data);}static
function
null($value){return'';}}}namespace Nette{use
Nette;final
class
ArrayTools{final
function
...
function
grep(array$arr,$pattern,$flags=0){Debug::tryError();$res=preg_grep($pattern,$arr,$flags);String::catchPregError($pattern);return$res;}}final
class
Callback
extends
...
__toString(){is_callable($this->cb,TRUE,$textual);return$textual;}}use
...
new\InvalidStateException('Directory to search has already been specified.');}if(!is_array($path)){$path=func_get_args();}$this->paths=$path;$this->cursor=&$this->exclude;return$this;}function
...
new\InvalidStateException('Call in() or from() to specify directory to search.');}elseif(count($this->paths)===1){return$this->buildIterator($this->paths[0]);}else{$iterator=new\AppendIterator();foreach($this->paths
...
class
Framework{const
NAME='Nette Framework',VERSION='2.0-dev',REVISION='7616569 released on 2011-03-10';public
static$iAmUsingBadHost=FALSE;final
function
__construct(){throw
new\LogicException("Cannot instantiate static class ".get_class($this));}}}namespace Nette\Web{use
Nette;class
Html
extends
Nette\Object
...
static;$parts=explode(' ',$name,2);$el->setName($parts[0]);if(is_array($attrs)){$el->attrs=$attrs;}elseif($attrs!==NULL){$el->setText($attrs);}if(isset($parts[1])){foreach(Nette\String::matchAll($parts[1].' ','#([a-z0-9:-]+)(?:=(["\'])?(.*?)(?(2)\\2|\s))?#i')as$m){$el->attrs[$m[1]]=isset($m[3])?$m[3]:TRUE;}}return$el;}final
function
...
Nette\GenericRecursiveIterator(new\ArrayIterator($this->children)),$deep);}else{return
new
Nette\GenericRecursiveIterator(new\ArrayIterator($this->children));}}final
function
...
as$key=>$value){if(is_object($value)){$this->children[$key]=clone$value;}}}}}namespace Nette{use
Nette;class
Image
extends
Object{const
...
new\Exception("PHP extension GD is not loaded.");}$info=@getimagesize($file);switch($format=$info[2]){case
self::JPEG:return
new
...
function
getFormatFromString($s){$types=array('image/jpeg'=>self::JPEG,'image/gif'=>self::GIF,'image/png'=>self::PNG);$type=Tools::detectMimeTypeFromString($s);return
...
new\InvalidArgumentException("Argument must be a string, ".gettype($input)." given.");}if(!self::$tokenizer){self::$tokenizer=new
...
new\MemberAccessException("Cannot read $type property $class::\$$name.");}static
function
...
new\MemberAccessException("Cannot write to $type property $class::\$$name.");}static
function
...
Paginator
extends
Object{private$base=1;private$itemsPerPage=1;private$page;private$itemCount;function
...
new\InvalidStateException("Callback '$textual' is not callable.");}$res=preg_replace_callback($pattern,$replacement,$subject,$limit);if(Debug::catchError($e)){$trace=$e->getTrace();if(isset($trace[2]['class'])&&$trace[2]['class']===__CLASS__){throw
new
...
getOffset($i){$tokens=String::split($this->input,$this->re,PREG_SPLIT_NO_EMPTY|PREG_SPLIT_OFFSET_CAPTURE);$offset=isset($tokens[$i])?$tokens[$i][1]:strlen($this->input);return
...
new\FileNotFoundException("File '$file' not found.");}$info=@getimagesize($file);if(isset($info['mime'])){return$info['mime'];}elseif(extension_loaded('fileinfo')){$type=preg_replace('#[\s;].*$#','',finfo_file(finfo_open(FILEINFO_MIME),$file));}elseif(function_exists('mime_content_type')){$type=mime_content_type($file);}return
...
new\InvalidStateException('Critical section has already been entered.');}$handle=substr(PHP_OS,0,3)==='WIN'?@fopen(NETTE_DIR.'/lockfile','w'):@fopen(__FILE__,'r');if(!$handle){throw
new\InvalidStateException("Unable initialize critical section.");}flock(self::$criticalSections=$handle,LOCK_EX);}static
function
leaveCriticalSection(){if(!self::$criticalSections){throw
new\InvalidStateException('Critical section has not been initialized.');}flock(self::$criticalSections,LOCK_UN);fclose(self::$criticalSections);self::$criticalSections=NULL;}}}